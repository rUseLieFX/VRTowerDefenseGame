...
namespace Valve.VR.InteractionSystem
{
	...
	public class Teleport : MonoBehaviour
    {
		...
		/// <summary>
		/// Saját eljárás! Ha problémázik, törlés!
		/// </summary>
		public void NewTeleportMarker(TeleportMarkerBase newTeleport)
		{
			List<TeleportMarkerBase> lista = new List<TeleportMarkerBase>();
            Array.ForEach(teleportMarkers, lista.Add);

            lista.Add(newTeleport);
			teleportMarkers = lista.ToArray();
		}

		/// <summary>
		/// Saját eljárás! Ha problémázik, törlés!
		/// </summary>
		public void DeletedTeleportMarker(TeleportMarkerBase deletedTeleport)
		{
            List<TeleportMarkerBase> lista = new List<TeleportMarkerBase>();

			foreach (var item in teleportMarkers)
			{
				if (item == deletedTeleport) continue;
				lista.Add(item);
			}
